...
export function buildGenuiComparisonPrompt(
  digests: RepoDigest[],
  dateStr: string,
  lang: Lang = "zh",
): string {
  ...
  if (lang === "en") {
    return `You are a senior technical analyst of the generative UI ecosystem. The following are ${dateStr} community digest summaries for each major generative UI project:
...
1. **Ecosystem Overview** - 3-5 sentences on the overall generative UI development landscape
2. **Activity Comparison** - Table comparing Issues count, PR count, Release status for each project today
3. **Shared Feature Directions** - Requirements appearing across multiple projects (note which projects, specific needs)
4. **Differentiation Analysis** - Differences in feature focus, target users, and technical approach
...
`;
  ...
  return `你是一位专注于生成式 UI 生态的资深技术分析师。以下是 ${dateStr} 各主流生成式 UI 项目的社区动态摘要：
...
1. **生态全景** - 用3-5句话概括当前生成式 UI 整体发展态势
